...
if [ "$CC_SKIP_PACKAGE_INSTALL" = "true" ]; then
  ...
  # peer0 of owner
  installChaincode 0 owner
  # peer1 of owner
  installChaincode 1 owner
  # peer2 of owner
  installChaincode 2 owner
  # peer0 of seller
  installChaincode 0 seller
  # peer1 of seller
  installChaincode 1 seller
  # peer2 of seller
  installChaincode 2 seller
fi
...
if [ "$CHANNEL_NAME" = "assemble" ]; then
  ...
  queryInstalled 0 owner

 ## approve the definition for config.orgs
  approveForMyOrg 0 owner
  approveForMyOrg 0 seller
  ## check whether the chaincode definition is ready to be committed
  ## expect all of them to have approved

  checkCommitReadiness 0 owner
  checkCommitReadiness 0 seller

  ## commit the definition
  commitChaincodeDefinition owner seller 

  ## query to see that the definition committed successfully
  queryCommitted 0 owner
  queryCommitted 0 seller
