...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating owner Identities"

    cryptogen generate --config=./organizations/cryptogen/crypto-config-owner.yaml --output="organizations"
    res=$?
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/owner/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    if [ "$OS_ARCH" == "darwin-amd64" ]; then
      ...
      rm -rf organizations/fabric-ca/owner
      rm -rf organizations/fabric-ca/seller
      ...
      ## remove fabric ca artifacts
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/owner'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/seller'
