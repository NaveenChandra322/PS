...
func (s *SmartContract) AddPrivateData(ctx contractapi.TransactionContextInterface) Response {
	...
	err = ctx.GetStub().PutPrivateData(ownerCollection, empPrivate.PropID, pvtJSONasBytes)

	if err != nil {
		return BuildResponse("ERROR", fmt.Sprintf("Failed to add Property private data to the blockchain"), nil)
	}
	return BuildResponse("SUCCESS", fmt.Sprintf("Property private data added to the blockchain successfully."), nil)
...
func (s *SmartContract) QueryPvtRecordByEmpId(ctx contractapi.TransactionContextInterface, args string) Response {
	...
	empPvtBytes, err := ctx.GetStub().GetPrivateData(ownerCollection, pvtObj.PropID)

	//No Asset found, return empty response
	if empPvtBytes == nil {
		return BuildResponse("ERROR", fmt.Sprintf("%v does not exist in collection %v", pvtObj.PropID, ownerCollection), nil)
	}
